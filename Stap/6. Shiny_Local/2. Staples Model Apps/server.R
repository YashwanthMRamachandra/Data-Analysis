...
shinyServer(function(input, output) {
  ...
  output$table <- renderDataTable({
    if(input$latency_DV=="Monthly"){Staples_Aggr_Monthly}
    else if(input$latency_DV=="Daily"){Staples_Daily}
    else if(input$latency_DV=="Weekly"){Staples_Aggr_Weekly}
    else{Staples_Aggr_Monthly_by_Prod}
  })
  
  # Correlation
  output$cor <- renderTable({
      if(input$latency_Cor=="Monthly"){data1 = data.frame(subset(Staples_Aggr_Monthly,Loyalty_Cat==input$Demand_Loyalty))
      cor(data1[,c("Conversion","Price_Comp")])}
      else if(input$latency_Cor=="Daily"){data2 = data.frame(subset(Staples_Aggr_Daily,Loyalty_Cat==input$Demand_Loyalty))
      cor(data2[,c("Conversion","Price_Comp")])}
      else if(input$latency_Cor=="Weekly"){data3 = data.frame(subset(Staples_Aggr_Weekly,Loyalty_Cat==input$Demand_Loyalty))
