...
rm(list=setdiff(ls(),c("Review_Aggr_Data","SA_Input")))
Review_Aggr_Model_Data <- read.table("C:/Yashwanth/Sports Authority/3. Model/3. Model_Input_by_Cat_All_Reviews.csv",
                            header = TRUE, sep = ",", quote = "\"", dec = "." ,fill=TRUE, comment.char="", as.is=TRUE)

#---------------------------------------------------------------------------------------------------------------
#                                     Select Time Period & segregate data on Quarterly basis
#---------------------------------------------------------------------------------------------------------------

SA_Input$Review_Creation_Date <- as.Date(SA_Input$Review_Creation_Date)
SA_Input_Date <- subset(SA_Input,Review_Creation_Date>="2014-04-01" & Review_Creation_Date<="2015-03-31",names(SA_Input))

SA_Input_Date$Review_Count <- as.numeric(SA_Input_Date$Review_Count)
SA_Input_Date$Review_Count[is.na(SA_Input_Date$Review_Count)] <- 0
...
#   assign(SA_Input[i], read.csv(SA_Input[i],stringsAsFactors = FALSE,header = TRUE, sep = ",", quote = "\"", fill=TRUE, 
...
# str(SA_Input);# write.csv(SA_Input,"SA_Input.csv",row.names = F)
