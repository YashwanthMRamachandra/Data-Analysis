...
system.time(  
  final.df <- read.table("C:/Yashwanth/OC & C/Input_file/Uplift Data - detail CTG Bain Adjust Frequency cluster CZ v2.txt",
...
df1 <- sqldf("select df.CL1,df.Frequency_cluster,df.catman_buy_domain_desc, 
             ...
             GROUP BY df.CL1,df.Frequency_cluster,df.catman_buy_domain_desc, 
             df.Cat_Ident, df.OCC_cat_group")

write.csv(df1,"C:/Yashwanth/OC & C/Reports/20140812/Final Reports_Rcode_Promo_CL1+Frequency_Cluster_uplift_20140813/Stage1.csv")


df2 <- sqldf("select df.CL1,df.Frequency_cluster,
             sum(df.PROM_sales) as PROM_sales,
             sum(df.PROM_profit) as PROM_profit,
             sum(df.PROM_LI_BAS) as PROM_LI_BAS,
             sum(df.PROM_LI_SAP) as PROM_LI_SAP,
             ...
             sum(df.PREV_duration) as PREV_duration
             from df1 as df 
             GROUP BY df.CL1,df.Frequency_cluster")

write.csv(df2,"C:/Yashwanth/OC & C/Reports/20140812/Final Reports_Rcode_Promo_CL1+Frequency_Cluster_uplift_20140813/Stage2.csv")


df3 <- sqldf("select df.CL1,
             sum(df.PROM_sales) as PROM_sales,
             sum(df.PROM_profit) as PROM_profit,
             sum(df.PROM_LI_BAS) as PROM_LI_BAS,
             sum(df.PROM_LI_SAP) as PROM_LI_SAP,
...
write.csv(df3,"C:/Yashwanth/OC & C/Reports/20140812/Final Reports_Rcode_Promo_CL1+Frequency_Cluster_uplift_20140813/Stage3.csv")
